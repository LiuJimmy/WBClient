...
public class StringUtil {
    ...
    public static SpannableString getWeiboContent(final Context context, final TextView tv,
                                                  String source, boolean clickable) {
        ...
        for (; ; ) { // 如果可以匹配到
            if (matcherLink.find()) {
                    ...
                    TouchableSpan clickableSpan = new TouchableSpan(
                        ...
                        public void onClick(View widget) {
                            if (key.startsWith("@")) {
                                ...
                                ToastUtil.show(context, "查看话题 :" + key);
                ...
                Bitmap bitmap = HttpUtil.getEmojiByName(context, key);
                if (bitmap != null && size > 0) {
                    //根据TextView的大小压缩表情
                    bitmap = Bitmap.createScaledBitmap(bitmap, size, size, true);

                    ImageSpan span = new ImageSpan(context, bitmap);
                    spannableString.setSpan(span, start, start + key.length(), Spannable
